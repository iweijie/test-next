...
function enhanceRedux(models = [], options = {}) {
    let { enhancer = [], reducer, states = {}, separator = NAMESPACE_SEP } = options;
    ...
    function registerModel(model) {
        ...
        if (isObject(reducers)) {
            Object.keys(reducers).forEach((key) => {
                if (!decorateReducers[namespace])
                    decorateReducers[namespace] = {};
                if (!primevalReducer[namespace])
                    primevalReducer[namespace] = {};
    ...
    function defaultReduce(state = {}, action = {}) {
        ...
        if (!action || !action.type || isReduxPrimitiveType(action.type)) return state;
        if (!isProduction) {
